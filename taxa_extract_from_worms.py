...
import pathlib
import datetime
import worms_rest_api_client


class SharkSpeciesListGenerator:
    """ Creates a list of taxa from biota down to subspecies, etc., 
    """
    ...
    def __init__(self):
        ...
        self.worms_client = worms_rest_api_client.WormsRestApiClient()
        #
    ...
    def run_all(self):
        ...
        counter = 0
        counter_max = len(self.indata_species_list)
        for species in sorted(self.indata_species_list):
            ...
            if species in self.old_translate_worms_dict:
                scientific_name = self.old_translate_worms_dict[species].get(
                    "scientific_name_to", ""
                )
                ...
                if aphia_id:
                    ...
                    if error:
                        ...
                        if aphia_id == valid_aphia_id:
                            ...
                            # aphia_id, error = self.worms_client.get_aphia_id_by_name(valid_name)
                        ...
                        # Step 6. Add children to classification dictionary.
                        if valid_aphia_id:
                            get_all_children(self, valid_aphia_id, valid_name)
    ...
    def import_species_list(self):
        ...
        not_valid_taxa_path = pathlib.Path("data_out/indata_not_valid_names.txt")
        not_valid_taxa_file = not_valid_taxa_path.open("w")
        not_valid_taxa_file.write("scientific_name" + "\n")
        ...
        with taxa_worms_file.open(
            ...
            for taxa_rec in self.translate_to_worms_dict.values():
                ...
                try:
                    ...
                    try:
                        print(
                            "Exception when writing to translate_to_worms.txt: ",
                            row[0],
                            "   ",
                            e,
                        )
    ...
    def save_errors(self):
        ...
        with errors_file.open("w", encoding="cp1252", errors="ignore") as outdata_file:
            ...
            for row in self.errors_list:
                out_row = []
                for row_item in row:
                    out_row.append(str(row_item))
                try:
                    outdata_file.write("\t".join(out_row) + "\n")
                except Exception as e:
                    try:
                        print(
                            "Exception when writing to errors.txt: ", row[0], "   ", e
                        )
                    except:
                        pass


def get_all_children(master_object, valid_aphia_id, valid_name):
    if valid_aphia_id:
        worms_records, error = master_object.worms_client.get_children_by_aphiaid(
            valid_aphia_id
        )
        if error:
            master_object.errors_list.append(["", valid_aphia_id, error])
        if len(worms_records) > 0:
            for worms_rec in worms_records:
                status = worms_rec.get("status", "")
                if status == "accepted":
                    # Replace "None" by space.
                    for key in worms_rec.keys():
                        if worms_rec[key] in ["None", None]:
                    ...
                    for (
                        from_key,
                        to_key,
                    ) in master_object.rename_worms_header_items.items():
                        worms_rec[to_key] = worms_rec.get(from_key, "")
                    #
                    worms_rec["parent_id"] = valid_aphia_id
                    worms_rec["parent_name"] = valid_name

                    child_aphia_id = worms_rec.get("aphia_id", "")
                    child_scientific_name = worms_rec.get("scientific_name", "")

                    master_object.taxa_worms_dict[child_scientific_name] = worms_rec

                    get_all_children(
                        master_object, child_aphia_id, child_scientific_name
                    )
